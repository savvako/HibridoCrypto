...
public final class TwofishAlgorithm {
    ...
    public static void main(String[] args) {
        try {
            ...
            for (int i = 0; i < BLOCK_SIZE; i++)
                pt[i] = (byte) i;
            TwofishAlgorithm tf = new TwofishAlgorithm(TwofishAlgorithm.generateKey());
            byte[] ct = tf.encrypt(pt);
            byte[] cpt = tf.decrypt(ct);
